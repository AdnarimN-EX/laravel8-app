...
@section('citizenIndex')
    ...
    <span class="text-sm text-gray-700 dark:text-gray-400">
        Showing <span class="font-semibold text-gray-900">{{ $citizens->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $citizens->lastItem() }}</span> of <span class="font-semibold text-gray-900">{{ $citizens->total() }}</span> Records
    </span>
    
    <div class="inline-flex mt-2 xs:mt-0">
        @if ($citizens->onFirstPage())
            <button class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-400 rounded-l-lg cursor-not-allowed">
                Prev
            </button>
        @else
            <a href="{{ $citizens->previousPageUrl() }}" class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-800 rounded-l-lg hover:bg-gray-900">
                Prev
            </a>
        @endif

        @if ($citizens->hasMorePages())
            <a href="{{ $citizens->nextPageUrl() }}" class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-800 rounded-r-lg border-0 border-s border-gray-700 hover:bg-gray-900">
                Next
            </a>
        @else
            <button class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-400 border-0 border-s border-gray-700 rounded-r-lg cursor-not-allowed dark:bg-gray-800 dark:text-gray-400">
                Next
            </button>
        @endif
    </div>
        ...
        <form action="{{ route('citizens.index') }}" class="mb-6 pt-6" method="GET">
            <h5 id="drawer-right-label"
                class="items-center mb-5 text-base font-bold text-gray-500 dark:text-gray-400">
                Filter Search
            </h5>
